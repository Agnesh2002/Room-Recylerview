...
class CustomAdapter(val list: List<UserData>) : RecyclerView.Adapter<CustomAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        holder.text1.text = list.get(position).id.toString()
        holder.text2.text = list.get(position).username
    }
